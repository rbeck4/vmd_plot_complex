...
set isoval 0.008
set path "./cubs/"
set file_root "casscf_sa50_50s_2.mo_trans_test.gb3lyp_x2cso_ref.yb3p.exp.tzvpall.gdv_j14p.7611325.mo_"
set file_ext ".cube"
...
material change ambient GlassBubble 0.30
material change diffuse GlassBubble 0.80
material change specular GlassBubble 0.50
material change shininess GlassBubble 0.40
material change mirror GlassBubble 0.000000
material change opacity GlassBubble 0.50
...
for {set i $mo_init} {$i < [expr $mo_final + 1]} {incr i $step} {
    ...
    foreach j {"a" "b"} {
        ...
        #REAL ISO:
        mol addfile ${path}${file_root}${i}.cubetwoc_r${j}${file_ext} type {cube} first 0 last -1 step 1 waitfor 1 volsets {0 } $mid
        mol addrep top
        mol modstyle 4 $mid Isosurface 0.015 2 0 0
        mol modcolor 4 $mid ColorID 1
        mol modmaterial 4 $mid Diffuse
        mol addrep top
        mol modstyle 5 $mid  Isosurface -0.015 2 0 0
    ...
    render Tachyon /tmp/render.tach tachyon -res $res $res -aasamples 5 /tmp/render.tach -format BMP -o ${path}/${file_root}_${i}_real.bmp
